...
import { useStorefront, useFilesystem } from 'hooks';
    ...
    return (
        <View style={[tailwind('z-50'), props.style || {}]}>
            <View style={tailwind('flex h-40 overflow-hidden')}>
                <ImageBackground source={{ uri: getUrl(props.info.backdrop_url)}} 
                    style={tailwind('flex-1 relative')} imageStyle={tailwind('bg-cover absolute -bottom-12')}
                >
