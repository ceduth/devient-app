...
import { useStorefront, useLocale, useFilesystem } from 'hooks';
...
const StoreSearch = ({ store, wrapperStyle, buttonTitle, buttonStyle, buttonTitleStyle, buttonIcon, buttonIconSize, buttonIconStyle, numberOfLines, onResultPress }) => {
    ...
    const fetchResultsFromStore = async (query, cb) => {
        ...
        */
        ...
        /* DIDNT WORK
        const results = await storefront.search({ 'attributes.name': query }, { store: store?.id }).catch(logError);
        const results = await storefront.products.query() => lists all products !
        */        // console.log('\n>>>>>>> StoreSearch store.id=', storefront.search, store.id, query, 'results=', results.length)


        // HACK
        let results = await storefront.products.findAll();
                                    ...
                                    <View style={tailwind('flex flex-row')}>
                                        <View style={tailwind('mr-3')}>
                                            <View style={tailwind('border border-gray-200 shadow-sm')}>
                                                <Image source={{ uri: getUrl(product.getAttribute('primary_image_url')) }} 
