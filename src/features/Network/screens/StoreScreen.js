...
import { useLocale, useMountedState, useFilesystem } from 'hooks';
...
const StoreScreen = ({ navigation, route }) => {
    ...
    const hasContactInformation = store.isAttributeFilled(['phone']) 
    ...
    const shareStore = useCallback(() => {
        ...
        const storeName = store.getAttribute('website')
        const websiteLink = store.getAttribute('website')

        Share.open({
            title: translate('Network.StoreScreen.shareStoreTitleText', { storeName, networkName, websiteLink }),
            subject: translate('Network.StoreScreen.shareStoreSubjectText', { storeName, networkName, websiteLink }),
            message: translate('Network.StoreScreen.shareStoreMessageText', { storeName, networkName, websiteLink }),
        });
    });
                        ...
                        <View style={tailwind('flex flex-row p-3 rounded-md mb-2')}>
                            <View style={tailwind('mr-3')}>
                                <View style={tailwind('rounded-md')}>
                                    <FastImage source={{ uri: getUrl(store.getAttribute('logo_url')) }} style={tailwind('h-18 w-18 rounded-md')} />
                                </View>
                            </View>
    ...
    return (
        <ImageBackground 
            source={{ uri: getUrl(store.getAttribute('backdrop_url')) }} 
            style={tailwind('bg-gray-100 h-full')} imageStyle={tailwind('bg-cover')}
        >
